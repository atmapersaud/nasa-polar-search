...
def main():
    ...
    fname_dict = {fname:i+1 for i,fname in enumerate(links)}
            
    d3result = {'nodes':[], 'links':[]}

    for fname in links:
        d3result['nodes'].append({'name':fname, 'group':1})
        for other in links[fname]:
            d3result['links'].append({'source':fname_dict[fname], 'target':fname_dict[other], 'value':1})

    with open(d3File, 'w') as d3f:
        json.dump(d3result, d3f)

    with open(pageRankFile, 'w') as prf:
        for fname in ranks:
            prf.write(fname + ' ' + str(ranks[fname]) + '\n')
