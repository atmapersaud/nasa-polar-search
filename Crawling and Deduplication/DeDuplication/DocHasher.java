...
public class DocHasher
{
     ...
     *@see: URLData class
     */
    public List< Set<Integer> > readAndHashDocs() {
	BufferedReader br=null;
	NearDeduplicator ndd = new NearDeduplicator();
	List<String> urls = new ArrayList<String>();
	List< Set<Integer> > docHashes = new ArrayList< Set<Integer> >();
	try {
		...
		if(cr.contains("URL::")) {
		    if(udata!=null) {
			Set<Integer> docHash = ndd.getMinHash(udata);
			if (docHash.size() > 0) {			    
			    docHashes.add(docHash);
			    this.docUrls.add(udata.getURL());
			}
		    }
